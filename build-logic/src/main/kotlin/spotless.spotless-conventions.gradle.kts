...
// Clones that lack origin/main (e.g. a single-branch snapshot) ratchet from HEAD instead of
// failing with "No such reference", so only uncommitted changes are checked.
fun ratchetRef(): String {
  val ref = "origin/main"
  val gitDir = rootDir.resolve(".git")
  if (!gitDir.isDirectory) return ref
  if (gitDir.resolve("refs/remotes/$ref").isFile) return ref
  val refs = gitDir.resolve("packed-refs")
  val packed = refs.isFile && refs.readLines().any { it.endsWith(" refs/remotes/$ref") }
  return if (packed) ref else "HEAD"
}
